...
export default function Button({ children, variant = "primary", size = "md", loading = false, disabled = false, className = "", onClick, type = "button", }) {
    ...
    return (_jsxs(motion.button, { type: type, onClick: onClick, disabled: disabled || loading, whileHover: disabled || loading ? {} : { y: -2 }, whileTap: disabled || loading ? {} : { scale: 0.97 }, transition: { duration: 0.18, ease: "easeOut" }, className: [
            // Base
            ...
            sizes[size],
            className,
        ].join(" "), children: [variant === "primary" && !disabled && !loading && (_jsx("span", { "aria-hidden": true, className: "absolute inset-0 -translate-x-full hover:translate-x-full transition-transform duration-700 ease-in-out bg-gradient-to-r from-transparent via-white/20 to-transparent pointer-events-none" })), loading ? (_jsx(Loader2, { className: "w-4 h-4 animate-spin shrink-0" })) : null, children] }));
}
